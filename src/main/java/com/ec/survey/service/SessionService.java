...
public class SessionService extends BasicService {
	...
	public Form getFormFromSessionInfo(HttpServletRequest request) throws Exception
	{
		...
		{		
			...
			if (!survey.getLanguage().getCode().equalsIgnoreCase(languageCode))
			{
	...
	public boolean userIsFormAdmin(Survey survey, User user, HttpServletRequest request) 
	{
		...
		try {
			...
			if (user.getLocalPrivileges().get(LocalPrivilege.FormManagement) == 2) return true;
		} catch (Exception e) {
	...
	public boolean userIsFormManager(Survey survey, User user, HttpServletRequest request)
	{
		...
		try {
			upgradePrivileges(survey, user, request);
			if (user.getLocalPrivileges().get(LocalPrivilege.FormManagement) > 0) return true;
		} catch (Exception e) {
			logger.error(e.getLocalizedMessage(), e);
		}		
	...
	public void upgradePrivileges(Survey survey, User user, HttpServletRequest request) throws Exception
	{
	...
	public void updateSessionInfo(Survey survey, User user, HttpServletRequest request)
	{
		...
		if (info != null)
		{
			if (info.getSurvey() != survey.getId())
			{
	...
	public SurveyFilter getSurveyFilter(HttpServletRequest request, boolean forms) throws NotAgreedToTosException, WeakAuthenticationException, NotAgreedToPsException {
		...
		if (request.getParameter("clearFilter") != null && "true".equals(request.getParameter("clearFilter"))) {
			if (forms) request.getSession().removeAttribute("SURVEYFILTER");
			return filter;
		}
		...
		if (request.getParameter("itemsPerPage") != null)
		{
			...
			if (filterPublished) status += "Published;";
	    	...
	    	if (forms) request.getSession().setAttribute("SURVEYFILTER", filter);
	...
	@Transactional
	public void setLastResultFilter(HttpServletRequest request, ResultFilter filter, int user, int survey) throws Exception {
	 ...
	 * @return session attribute if user need to call ajax function to check 
	 * for new exports
	 */
