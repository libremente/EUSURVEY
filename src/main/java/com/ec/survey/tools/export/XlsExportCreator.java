...
public class XlsExportCreator extends ExportCreator {
	...
	void exportStatistics() throws Exception {
		...
		if (filter != null) visibleQuestions = filter.getExportedQuestions();
		if (visibleQuestions == null || visibleQuestions.size() == 0) visibleQuestions = filter.getVisibleQuestions();
			
        for (Element question : survey.getQuestionsAndSections()) {
        	
        	if (filter == null || visibleQuestions.size() == 0 || visibleQuestions.contains(question.getId().toString()))
        	{
				...
				if (question instanceof ChoiceQuestion)
				{
					...
					for (PossibleAnswer possibleAnswer : choiceQuestion.getAllPossibleAnswers()) {
					   ...
					   {
						   row.createCell(2).setCellValue(statistics.getRequestedRecords().get(possibleAnswer.getId().toString()));
					   }
					   Cell pcell = row.createCell(3);
					   pcell.setCellValue(percent / 100);   //df.format(percent) + "%");
					...
					for (int i = 0; i < galleryQuestion.getFiles().size(); i++) {
						...
						row.createCell(2).setCellValue(statistics.getRequestedRecords().get(galleryQuestion.getId().toString() + "-" + i));
						
						Cell pcell = row.createCell(3);
						pcell.setCellValue(statistics.getRequestedRecordsPercent().get(galleryQuestion.getId().toString() + "-" + i)  / 100);  
						pcell.setCellStyle(percentStyle);
					}					
					...
					for (Element matrixQuestion: matrix.getQuestions()) {
						...
						for (Element matrixAnswer: matrix.getAnswers()) {
							...
							row.createCell(2).setCellValue(statistics.getRequestedRecordsForMatrix(matrixQuestion, matrixAnswer));
							
							Cell pcell = row.createCell(3);
							pcell.setCellValue(statistics.getRequestedRecordsPercentForMatrix(matrixQuestion, matrixAnswer) / 100);  
							pcell.setCellStyle(percentStyle);
						}
						...
						row.createCell(2).setCellValue(statistics.getRequestedRecords().get(matrixQuestion.getId().toString()));
						
						Cell pcell = row.createCell(3);
						pcell.setCellValue(statistics.getRequestedRecordsPercent().get(matrixQuestion.getId().toString()) / 100);  
						pcell.setCellStyle(percentStyle);
					...
					for (Element childQuestion: rating.getQuestions()) {
						...
						for (int i = 1; i <= rating.getNumIcons(); i++) {
							...
							row.createCell(2).setCellValue(statistics.getRequestedRecordsForRatingQuestion(childQuestion, i));
							
							Cell pcell = row.createCell(3);
							pcell.setCellValue(statistics.getRequestedRecordsPercentForRatingQuestion(childQuestion, i) / 100);  
							pcell.setCellStyle(percentStyle);
						}
						...
						row.createCell(2).setCellValue(statistics.getRequestedRecords().get(childQuestion.getId().toString()));
						
						Cell pcell = row.createCell(3);
						pcell.setCellValue(statistics.getRequestedRecordsPercent().get(childQuestion.getId().toString()) / 100);  
						pcell.setCellStyle(percentStyle);
        ...
        logger.info("exporting statistics at " + exportFilePath);
        wb.write(outputStream);
	}
	...
	void exportActivities() throws Exception {
		...
		{	
			...
			{				
				...
				{
					cell = row.createCell(cellIndex++);
					cell.setCellValue(resources.getMessage("logging." + activity.getLogID(), null, "logging." + activity.getLogID(), locale));
				}
	...
	private void exportOneECFGlobalResultSheet(ECFGlobalResult globalResult) {
		...
		for (ECFGlobalCompetencyResult competencyResult : globalResult.getIndividualResults()) {
			row = sheet.createRow(rowIndex++);
			columnIndex = 0;
			logger.info("created a row at index " + rowIndex);
			...
			logger.info("created a cell for competencyName at index" + columnIndex);
			...
			for (int i = 0; i<competencyResult.getCompetencyScores().size(); i++) {
				...
				logger.info("created a cell for score" + score + " at index" + columnIndex);
			}
		}
	}
