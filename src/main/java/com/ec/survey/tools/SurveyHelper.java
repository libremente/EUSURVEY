...
public class SurveyHelper {
	...
	public static AnswerSet parseAnswerSet(HttpServletRequest request, Survey survey, String fileDir, String uniqueCode, boolean update, String languageCode, User user, FileService fileService)
	{
		...
		if (!survey.isAnonymous())
		{
			...
			if (request.getHeader("X-Forwarded-For") != null && !request.getHeader("X-Forwarded-For").equalsIgnoreCase("0.0.0.0")) ip += " (" + request.getHeader("X-Forwarded-For") + ")";
			answerSet.setIP(ip);
		}
		
		if (!update) answerSet.setDate(answerSet.getUpdateDate());
				...
				if (key.contains("|"))
				{
					...
					if (question != null) {
						...
						if (question instanceof Upload)
						{
							...
							java.io.File directory = new java.io.File(rootfolder.getPath() + "/" + uniqueCode + "/" + question.getId());
							...
							for (String value : values) {
								if (value.isEmpty() || value.equalsIgnoreCase("false") || (question instanceof DateQuestion && value.equalsIgnoreCase("DD/MM/YYYY"))) {
									continue;
								}
								Answer answer = new Answer();
								answer.setAnswerSet(answerSet);
	...
	public static HashMap<Element, String> validateAnswerSet(AnswerSet answerSet, AnswerService answerService, Set<String> invisibleElements, MessageSource resources, Locale locale, String draftid, HttpServletRequest request, boolean skipDraftCreation, User user, FileService fileService, boolean forpdf) throws Exception
	{
		...
		for (Element element : answerSet.getSurvey().getElements()) {
			...
			{
				validateElement(element, answerSet, dependencies, result, answerService, invisibleElements, resources, locale, null, request, draft);
				...
				if (element instanceof Matrix)
				{
					...
					for (Element matrixquestion : m.getQuestions())
					{
						...
						validateElement(matrixquestion, answerSet, dependencies, result, answerService, invisibleElements, resources, locale, m, request, draft);
			...
			} else if (element instanceof RatingQuestion)
			{
				...
				for (Element childquestion : r.getQuestions())
				{
					...
					validateElement(childquestion, answerSet, dependencies, result, answerService, invisibleElements, resources, locale, r, request, draft);
		...
		if (!skipDraftCreation && result.size() > 0)
		{
			...
			if (draft != null) {
				SurveyHelper.parseAndMergeAnswerSet(request, answerSet.getSurvey(), answerService.getFileDir(), answerSet.getUniqueCode(), draft.getAnswerSet(), answerSet.getLanguageCode(), user, fileService);
				draft.getAnswerSet().setIsDraft(true); // this also sets the ISDRAFT flag of the answers inside the answerset
	...
	public static boolean validateElement(Element element, AnswerSet answerSet, HashMap<Element, List<Element>> dependencies, HashMap<Element, String> result,  AnswerService answerService, Set<String> invisibleElements, MessageSource resources, Locale locale, Element parentElement, HttpServletRequest request, Draft draft)
	{	
		try {
			...
			Matrix parent = null;
			if (parentElement != null && parentElement instanceof Matrix) parent = (Matrix)parentElement;
			...
			if (element instanceof Question)
			{
				...
				if (parent == null ? question.getIsDependent() : question.getIsDependentMatrixQuestion())
				{
					...
					if (deps != null) questiondependencies = new ArrayList<>(deps);
					if (question instanceof Matrix)
					{
					...
					if (questiondependencies == null && parent != null)
					{
						...
						deps = dependencies.get(parent);
						if (deps != null) questiondependencies.addAll(deps);
					}
					...
					{						
						if (trigger instanceof Matrix)
						{
							...
							{	
								...
								if (dependentElementUniqueIds.contains(question.getUniqueId()) || (parent != null && dependentElementUniqueIds.contains(parent.getUniqueId())))
								{
									Element matrixQuestion = m.getQuestions().get(candidate.getPosition() / (m.getColumns()-1));
									Element matrixAnswer = m.getAnswers().get(candidate.getPosition() % (m.getColumns()-1) );
									if (answerSet.getMatrixAnswer(matrixQuestion.getId(), matrixAnswer.getId()) != null || answerSet.getMatrixAnswer(matrixQuestion.getUniqueId(), matrixAnswer.getUniqueId()) != null)
									{
										if (!invisibleElements.contains(matrixQuestion.getUniqueId()))
										{
											found = true;
							...
							if (trigger instanceof PossibleAnswer)
							{
								...
								for (Answer answer: answerSet.getAnswers(possibleAnswer.getQuestionId(), answerSet.getSurvey().getElementsById().get(possibleAnswer.getQuestionId()).getUniqueId()))
								{
									if (answer.getPossibleAnswerId().equals(trigger.getId()) || (answer.getPossibleAnswerUniqueId() != null && answer.getPossibleAnswerUniqueId().equals(trigger.getUniqueId())))
									{
				...
				if (element instanceof FreeTextQuestion)
				{
					...
					String answer = "";
					if (answers.size() > 0) answer = answers.get(0).getValue();
					
					if (!(freeTextQuestion.getIsPassword() && answer.equals("********")))
					{
						if (freeTextQuestion.getMinCharacters() > 0 && answer.length() > 0 && answer.length() < freeTextQuestion.getMinCharacters())
						{
							result.put(element,resources.getMessage("validation.textNotLongEnough", null, "This text is not long enough", locale));
						}
						
						if (freeTextQuestion.getMaxCharacters() > 0 && answer.length() > 0 && answer.length() > freeTextQuestion.getMaxCharacters())
						{
							result.put(element,resources.getMessage("validation.textTooLong", null, "This text is too long", locale));
						}
					}
					
					if (freeTextQuestion.getIsPassword() && freeTextQuestion.getIsComparable() && answer.equals("********"))
					{
						...
						if (draft != null)
						{
							...
							if (originalAnswers.size() > 0)
							{
								...
								if (currentPasswordAnswers.size() > 0)
								{
									...
									if (currentAnswer != null && currentAnswer.equalsIgnoreCase("********") && !first.equals(second))
									{
										result.put(element,resources.getMessage("validation.nomatch", null, "The two values do not match", locale));
									}
								}
							}							
						}
					...
					if (answerSet.getSurvey().getRegistrationForm() && question.getAttributeName().equalsIgnoreCase("email"))
					{
						if (!MailService.isValidEmailAddress(answer))
						{
							result.put(element,resources.getMessage("validation.invalidEmail", null, "This is not a valid email address", locale));
						}
					}			
					
					if (freeTextQuestion.getIsUnique())
					{
						int existing = answerService.getNumberAnswersForValue(answer, freeTextQuestion.getId(), freeTextQuestion.getUniqueId(), answerSet.getSurvey().getIsDraft(), answerSet.getUniqueCode());
						if (existing > 0) result.put(element,resources.getMessage("validation.notUnique", null, "This input already exists. Please try another entry.", locale));
					}
					...
					if (freeTextQuestion.getIsPassword() && freeTextQuestion.getSurvey().getShortname().equalsIgnoreCase("NewSelfRegistrationSurvey") && answer.length() > 0)
					{
						if (answer != null && !answer.equals("********") && Tools.isPasswordWeak(answer))
						{
							result.put(element,resources.getMessage("error.PasswordWeak", null, "This password does not fit our password policy. Please choose a password between 8 and 16 characters with at least one digit and one non-alphanumeric characters (e.g. !?$%...).", locale));
						}
					}
				...
				if (element instanceof RegExQuestion)
				{
					...
					{
						int existing = answerService.getNumberAnswersForValue(answer, regExQuestion.getId(), regExQuestion.getUniqueId(), answerSet.getSurvey().getIsDraft(), answerSet.getUniqueCode());
						if (existing > 0) result.put(element,resources.getMessage("validation.notUnique", null, "This input already exists. Please try another entry.", locale));
					}
				}
				...
				if (element instanceof NumberQuestion)
				{
					...
					try {
						if (answers.size() > 0 && answers.get(0).getValue().length() > 0)
						{
							...
							{
								result.put(element,resources.getMessage("validation.valueTooSmall", null, "This value is too small", locale));
							}
							
							if (numberQuestion.getMax() != null && answer != 0 && answer > numberQuestion.getMax())
							{
								result.put(element,resources.getMessage("validation.valueTooBig", null, "This value is too big", locale));
							}
							
							if (numberQuestion.getIsUnique())
							{
								int existing = answerService.getNumberAnswersForValue(answers.get(0).getValue().trim(), numberQuestion.getId(), numberQuestion.getUniqueId(), answerSet.getSurvey().getIsDraft(), answerSet.getUniqueCode());
								if (existing > 0) result.put(element,resources.getMessage("validation.notUnique", null, "This input already exists. Please try another entry.", locale));
							}
						}					
					} catch (Exception e)
					{
						result.put(element,resources.getMessage("validation.invalidNumber", null, "This value is not a valid number", locale));
					}					
				}
				...
				if (element instanceof DateQuestion)
				{
					...
					Date answer = null;
					if (answers.size() > 0 && answers.get(0).getValue().length() > 0) answer = ConversionTools.getDate(answers.get(0).getValue());
					
					if (dateQuestion.getMin() != null && answer != null && dateQuestion.getMin().compareTo(answer) > 0)
					{
						result.put(element,resources.getMessage("validation.valueTooSmall", null, "This value is too small", locale));
					}
					
					if (dateQuestion.getMax() != null && answer != null && dateQuestion.getMax().compareTo(answer) < 0)
					{
						result.put(element,resources.getMessage("validation.valueTooBig", null, "This value is too big", locale));
					}
				}
				...
				if (element instanceof MultipleChoiceQuestion)
				{
					...
					{
						result.put(element,resources.getMessage("validation.notEnoughAnswers", null, "Not enough answers selected", locale));
					}
					
					if (multipleChoiceQuestion.getMaxChoices() < answers.size() && multipleChoiceQuestion.getMaxChoices() > 0 && answers.size() > 0)
					{
						result.put(element,resources.getMessage("validation.tooManyAnswers", null, "Too many answers selected", locale));
					}				
				}
				...
				if (element instanceof Table)
				{
					...
					if (found)
					{
						result.put(element,resources.getMessage("validation.invalidCharacter", null, "Invalid (non UTF-8) characters detected. Please check your input.", locale));
					}
				}
					...
					{						
						if (trigger instanceof Matrix)
						{
							...
							for (DependencyItem candidate: m.getDependentElements())
							{
								...
								{
									if (dependent.getId().equals(section.getId()) || dependent.getUniqueId().equals(section.getUniqueId()))
									{
										Element matrixQuestion = m.getQuestions().get(candidate.getPosition() / (m.getColumns()-1));
										Element matrixAnswer = m.getAnswers().get(candidate.getPosition() % (m.getColumns()-1) );
										if (answerSet.getMatrixAnswer(matrixQuestion.getId(), matrixAnswer.getId()) != null || answerSet.getMatrixAnswer(matrixQuestion.getUniqueId(), matrixAnswer.getUniqueId()) != null)
							...
							if (trigger instanceof PossibleAnswer)
							{
								...
								for (Answer answer: answerSet.getAnswers(possibleAnswer.getQuestionId(), answerSet.getSurvey().getElementsById().get(possibleAnswer.getQuestionId()).getUniqueId()))
								{
									if (answer.getPossibleAnswerId().equals(trigger.getId()) || answer.getPossibleAnswerUniqueId().equals(trigger.getUniqueId()))
									{
										found = true;
									}
								}
	...
	public static AnswerSet parseAndMergeAnswerSet(HttpServletRequest request, Survey survey, String fileDir, String uniqueCode, AnswerSet answerSet, String languageCode, User user, FileService fileService)
	{
		...
		AnswerSet parsedAnswerSet = parseAnswerSet(request, survey, fileDir, uniqueCode, true, languageCode, user, fileService);
	...
	public static void recreateUploadedFiles(AnswerSet answerSet, String fileDir, Survey survey, FileService fileService)
	{
		...
		for (Answer answer: answerSet.getAnswers())
		{
			for (File file: answer.getFiles())
			{
				...
				try {
					...
					if (answer.getQuestionUniqueId() != null && answer.getQuestionUniqueId().length() > 0 && elementsByUniqueId.containsKey(answer.getQuestionUniqueId()))
					{
						questionId = elementsByUniqueId.get(answer.getQuestionUniqueId()).getId();
					}
					
					java.io.File directory = new java.io.File(fileService.getSurveyUploadsFolder(answerSet.getSurvey().getUniqueId(), false) + "/" + answerSet.getUniqueCode() + "/" + questionId);
				    directory.mkdirs();					        
	...
	private static Section getSection(Map<String, String[]> parameterMap, Element currentElement, Survey survey, String id, ServletContext servletContext, boolean log220) throws InvalidXHTMLException
	{
		Section section;
	...
	private static Text getText(Map<String, String[]> parameterMap, Element currentElement, Survey survey, String id, ServletContext servletContext, boolean log220) throws InvalidXHTMLException
	{
		String oldValues = "";
    	String newValues = "";
    	...
    	if (currentElement == null)
    	{
    		text = new Text(survey, getString(parameterMap, "text", id, servletContext), getString(parameterMap, "uid", id, servletContext));           	
    	} else {
	...
	private static Image getImage(Map<String, String[]> parameterMap, Element currentElement, Survey survey, String id, ServletContext servletContext, boolean log220) throws InvalidXHTMLException
	{
		Image image;
		String oldValues = "";
    	String newValues = "";
		...
		if (!url.contains("/")) url = servletContext.getContextPath() + "/files/" + url;
	...
	private static Upload getUpload(Map<String, String[]> parameterMap, Element currentElement, Survey survey, String id, ServletContext servletContext, boolean log220) throws InvalidXHTMLException
	{
		Upload upload;
		String oldValues = "";
    	String newValues = "";
		...
		if (currentElement == null)
		{
			upload = new Upload(survey, getString(parameterMap, "text", id, servletContext), getString(parameterMap, "shortname", id, servletContext), getString(parameterMap, "uid", id, servletContext));           	
		} else {
	...
	private static Download getDownload(Map<String, String[]> parameterMap, Element currentElement, Survey survey, String id, FileService fileService, ServletContext servletContext, boolean log220, Map<String, Integer> fileIDsByUID) throws InvalidXHTMLException
	{
		Download download;
		String oldValues = "";
    	String newValues = "";
	...
	private static Confirmation getConfirmation(Map<String, String[]> parameterMap, Element currentElement, Survey survey, String id, FileService fileService, ServletContext servletContext, boolean log220) throws InvalidXHTMLException
	{
	...
	private static FreeTextQuestion getFreeText(Map<String, String[]> parameterMap, Element currentElement, Survey survey, String id, ServletContext servletContext, boolean log220) throws InvalidXHTMLException
	{
		String oldValues = "";
    	String newValues = "";
	...
	private static RegExQuestion getRegEx(Map<String, String[]> parameterMap, Element currentElement, Survey survey, String id, ServletContext servletContext, boolean log220) throws InvalidXHTMLException
	{
		String oldValues = "";
    	String newValues = "";
		RegExQuestion regex;
	...
	private static EmailQuestion getEmail(Map<String, String[]> parameterMap, Element currentElement, Survey survey, String id, ServletContext servletContext, boolean log220) throws InvalidXHTMLException
	{
		EmailQuestion email;
		String oldValues = "";
    	String newValues = "";
	...
	private static NumberQuestion getNumber(Map<String, String[]> parameterMap, Element currentElement, Survey survey, String id, ServletContext servletContext, boolean log220) throws InvalidXHTMLException
	{
	...
	private static DateQuestion getDate(Map<String, String[]> parameterMap, Element currentElement, Survey survey, String id, ServletContext servletContext, boolean log220) throws InvalidXHTMLException
	{
		DateQuestion date;
		String oldValues = "";
    	String newValues = "";
		if (currentElement == null)
		{
			date = new DateQuestion(survey, getString(parameterMap, "text", id, servletContext), getString(parameterMap, "shortname", id, servletContext), getString(parameterMap, "uid", id, servletContext));          	
		} else {
	...
	private static RatingQuestion getRating(Map<String, String[]> parameterMap, Element currentElement, Survey survey, String id, ServletContext servletContext, boolean log220, String[] questions, String[] shortnamesForQuestions, String[] uidsForQuestions, String[] optionalsForQuestions) throws InvalidXHTMLException
	{
		...
		for (int k = 0; k < questions.length; k++) {
			...
			shortname = "";
			if (shortnamesForQuestions != null && shortnamesForQuestions.length > k) shortname = shortnamesForQuestions[k];
			...
			isOptional = true;
			if (optionalsForQuestions != null && optionalsForQuestions.length > k) isOptional = !optionalsForQuestions[k].equalsIgnoreCase("false");
			...
			if (!found) rating.getChildElements().add(q);						
		}
	...
	private static SingleChoiceQuestion getSingleChoice(Map<String, String[]> parameterMap, Element currentElement, Survey survey, String id, String[] answers, String[] dependenciesForAnswers, HashMap<PossibleAnswer, String> dependencies, String[] shortnamesForAnswers, String[] uidsForAnswers, String[] correctForAnswers, String[] pointsForAnswers, String[] feedbackForAnswers, ServletContext servletContext, boolean log220) throws InvalidXHTMLException
	{
		String oldValues = "";
    	String newValues = "";
    	...
    	{
			singlechoice = new SingleChoiceQuestion(survey, getString(parameterMap, "text", id, servletContext) , getString(parameterMap, "shortname", id, servletContext), getString(parameterMap, "uid", id, servletContext));         	
    	} else {
		...
		for (int k = 0; k < answers.length; k++) {
			...
			if (dependenciesForAnswers != null && dependenciesForAnswers.length > k) answerDependencies = dependenciesForAnswers[k];     
			shortname = "";
			if (shortnamesForAnswers != null && shortnamesForAnswers.length > k) shortname = shortnamesForAnswers[k];
			...
			if (survey.getIsQuiz())
			{
				...
				if (correctForAnswers != null && correctForAnswers.length > k) correct = correctForAnswers[k].equalsIgnoreCase("true");
				
				points = 0;
				if (pointsForAnswers != null && pointsForAnswers.length > k) points = Integer.parseInt(pointsForAnswers[k]);
				
				String feedback = "";
				if (feedbackForAnswers != null && feedbackForAnswers.length > k) feedback = feedbackForAnswers[k];			
			...
			if (!found) singlechoice.getPossibleAnswers().add(p);						
	...
	private static MultipleChoiceQuestion getMultipleChoice(Map<String, String[]> parameterMap, Element currentElement, Survey survey, String id, String[] answers, String[] dependenciesForAnswers, HashMap<PossibleAnswer, String> dependencies, String[] shortnamesForAnswers, String[] uidsForAnswers, String[] correctForAnswers, String[] pointsForAnswers, String[] feedbackForAnswers, ServletContext servletContext, boolean log220) throws InvalidXHTMLException
	{
		String oldValues = "";
    	String newValues = "";
    	...
    	{
			multiplechoice = new MultipleChoiceQuestion(survey, getString(parameterMap, "text", id, servletContext), getString(parameterMap, "shortname", id, servletContext), getString(parameterMap, "uid", id, servletContext));        	
    	} else {
		...
		for (int k = 0; k < answers.length; k++) {
			...
			if (!found) multiplechoice.getPossibleAnswers().add(p);
	...
	private static Matrix getMatrix(Map<String, String[]> parameterMap, Element currentElement, Survey survey, String id, Map<Integer, Element> elementsById, String[] dependenciesForAnswers, HashMap<Matrix, HashMap<Integer, String>> matrixDependencies, ServletContext servletContext, boolean log220) throws InvalidXHTMLException
	{
		String oldValues = "";
    	String newValues = "";    	
    	...
    	if (currentElement == null)
    	{
    		matrix = new Matrix(survey, getString(parameterMap, "text", id, servletContext), getString(parameterMap, "shortname", id, servletContext), getString(parameterMap, "uid", id, servletContext));
    	} else {
	...
	private static Table getTable(Map<String, String[]> parameterMap, Element currentElement, Survey survey, String id, Map<Integer, Element> elementsById, ServletContext servletContext, boolean log220) throws InvalidXHTMLException
	{
		String oldValues = "";
    	String newValues = "";    	
    	StringBuilder oldLabels = new StringBuilder();
    	...
    	if (currentElement == null)
    	{
    		table = new Table(survey, getString(parameterMap, "text", id, servletContext), getString(parameterMap, "shortname", id, servletContext), getString(parameterMap, "uid", id, servletContext));
    	} else {
	...
	public static Element parseElement(HttpServletRequest request, FileService fileService, String id, Survey survey, ServletContext servletContext, boolean log220) throws InvalidXHTMLException, IntrusionException
	{
		return parseElement(request, fileService, id, survey, new HashMap<>(), new HashMap<>() , new HashMap<>(), new HashMap<>(), servletContext, log220, new HashMap<>());
	}
	
	private static Element parseElement(HttpServletRequest request, FileService fileService, String id, Survey survey, Map<Integer, Element> elementsById, Map<Integer, Element> gridElementsById, HashMap<PossibleAnswer, String> dependencies, HashMap<Matrix, HashMap<Integer, String>> matrixDependencies, ServletContext servletContext, boolean log220, Map<String, Integer> fileIDsByUID) throws InvalidXHTMLException, IntrusionException
	{
		Map<String, String[]> parameterMap = Ucs2Utf8.requestToHashMap(request);
		...
		if (!id.contains("-") && elementsById.containsKey(Integer.parseInt(id)))
         ...
         {          	
         	element = getSection(parameterMap, currentElement, survey, id, servletContext, log220 && currentElement != null);            	
         } else if (type.equalsIgnoreCase("matrix"))
         {          	
         	String[] dependenciesForAnswers = parameterMap.get("dependencies" + id);
         	element = getMatrix(parameterMap, currentElement, survey, id, elementsById, dependenciesForAnswers, matrixDependencies, servletContext, log220 && currentElement != null);
         } else if (type.equalsIgnoreCase("table"))
         {          	
         	element = getTable(parameterMap, currentElement, survey, id, elementsById, servletContext, log220 && currentElement != null);	  
         } else if (type.equalsIgnoreCase("text"))
         {
         	element = getText(parameterMap, currentElement, survey, id, servletContext, log220 && currentElement != null);            	            	           	
         ...
         {
         	element = getDownload(parameterMap, currentElement, survey, id, fileService, servletContext, log220 && currentElement != null, fileIDsByUID);
         } else if (type.equalsIgnoreCase("confirmation"))
         {
         	element = getConfirmation(parameterMap, currentElement, survey, id, fileService, servletContext, log220 && currentElement != null);
         } else if (type.equalsIgnoreCase("freetext"))
         {
         	element = getFreeText(parameterMap, currentElement, survey, id, servletContext, log220 && currentElement != null);
         } else if (type.equalsIgnoreCase("regex"))
         {
         	element = getRegEx(parameterMap, currentElement, survey, id, servletContext, log220 && currentElement != null);
         } else if (type.equalsIgnoreCase("email"))
         {
         	element = getEmail(parameterMap, currentElement, survey, id, servletContext, log220 && currentElement != null);
         } else if (type.equalsIgnoreCase("number"))
         {
         	element = getNumber(parameterMap, currentElement, survey, id, servletContext, log220 && currentElement != null);
         } else if (type.equalsIgnoreCase("date"))
         {
         	element = getDate(parameterMap, currentElement, survey, id, servletContext, log220 && currentElement != null);
         } else if (type.equalsIgnoreCase("rating"))
         {
        	String[] questions = parameterMap.get("question" + id);
         	...
         	element = getRating(parameterMap, currentElement, survey, id, servletContext, log220 && currentElement != null, questions, shortnamesForQuestions, uidsForQuestions, optionalsForQuestions);
         } else if (type.equalsIgnoreCase("choice"))
         {
         	...
         	if (single)
         	{
         		element = getSingleChoice(parameterMap, currentElement, survey, id, answers, dependenciesForAnswers, dependencies, shortnamesForAnswers, uidsForAnswers, correctForAnswers, pointsForAnswers, feedbackForAnswers, servletContext, log220);
         	} else {
         		element = getMultipleChoice(parameterMap, currentElement, survey, id, answers, dependenciesForAnswers, dependencies, shortnamesForAnswers, uidsForAnswers, correctForAnswers, pointsForAnswers, feedbackForAnswers, servletContext, log220);    
         	}   
         }
	...
	private static ScoringItem getScoringItem(Map<String, String[]> parameterMap, Question element, String id, ServletContext servletContext,boolean log220) throws InvalidXHTMLException
	{
		...
		if (scoringItem == null)
		{
			if (element.getScoringItems() == null) element.setScoringItems(new ArrayList<>());
	...
	public static Survey parseSurvey(HttpServletRequest request, SurveyService surveyService, boolean draft, FileService fileService, ServletContext servletContext, boolean log217, boolean log220, Map<String, Integer> fileIDsByUID) throws InvalidXHTMLException, IntrusionException
	{
		...
		for(int i=0;i<ids.length;i++)
        {
            ...
            {
                Element element = parseElement(request, fileService, id, survey, elementsById, gridElementsById, dependencies, matrixDependencies, servletContext, log220, fileIDsByUID);
                if (element != null)
	            {
		            ...
		            if (!id.contains("-") && elementsById.containsKey(Integer.parseInt(id)))
		            {
		            	...
		            	{
		            		//this is for example a MultipleChoiceQuestion that was turned into a SingleChoiceQuestion
		...
		for (PossibleAnswer p : dependencies.keySet()) {
			...
			for (String questionID : questionIDs)
			{
				...
				{
					if ((element.getId() != null && element.getId().toString().equalsIgnoreCase(questionID)) || (element.getOldId() != null && element.getOldId().equalsIgnoreCase(questionID)))
					{
	...
	private static String getString(Map<String, String[]> parameterMap, String name, String id, ServletContext servletContext) throws InvalidXHTMLException
	{
		return getString(parameterMap, name, id, servletContext, false);
	}
	
	private static String getString(Map<String, String[]> parameterMap, String name, String id, ServletContext servletContext, boolean escape) throws InvalidXHTMLException
	{
		String result = null;
	...
	private static String[] getStrings(Map<String, String[]> parameterMap, String name, String id, ServletContext servletContext) throws InvalidXHTMLException
	{
		String[] result = null;
		try {
			if (parameterMap.get(name + id) != null)
	...
	public static void synchronizeSurvey(Survey survey, String languageCode, TranslationService translationService, Language language, boolean setSurvey)
	{
		if (language != null)
		{
		...
		{			
			...
			if (translationsByKey.get(Survey.TITLE) != null) survey.setTitle(translationsByKey.get(Survey.TITLE).getLabel());
			if (translationsByKey.get(Survey.INTRODUCTION) != null) survey.setIntroduction(translationsByKey.get(Survey.INTRODUCTION).getLabel());
			if (translationsByKey.get(Survey.ESCAPEPAGE) != null) survey.setEscapePage(translationsByKey.get(Survey.ESCAPEPAGE).getLabel());
			if (translationsByKey.get(Survey.ESCAPELINK) != null) survey.setEscapeLink(translationsByKey.get(Survey.ESCAPELINK).getLabel());
			if (translationsByKey.get(Survey.CONFIRMATIONPAGE) != null) survey.setConfirmationPage(translationsByKey.get(Survey.CONFIRMATIONPAGE).getLabel());
			if (translationsByKey.get(Survey.CONFIRMATIONLINK) != null) survey.setConfirmationLink(translationsByKey.get(Survey.CONFIRMATIONLINK).getLabel());
			if (translationsByKey.get(Survey.QUIZWELCOMEMESSAGE) != null) survey.setQuizWelcomeMessage(translationsByKey.get(Survey.QUIZWELCOMEMESSAGE).getLabel());
			if (translationsByKey.get(Survey.QUIZRESULTSMESSAGE) != null) survey.setQuizResultsMessage(translationsByKey.get(Survey.QUIZRESULTSMESSAGE).getLabel());
			...
			for (String key : survey.getUsefulLinks().keySet())
			{
				...
				if (translationsByKey.get(newkey) != null && translationsByKey.get(newkey).getLabel().length() > 0 && !(data[0] + "#" +translationsByKey.get(newkey).getLabel()).equals(key))
				{
					changes.put(data[0] + "#" + translationsByKey.get(newkey).getLabel(), survey.getUsefulLinks().get(key));
			...
			{				
				...
				if (translationsByKey.get(newkey) != null && translationsByKey.get(newkey).getLabel().length() > 0 && !translationsByKey.get(newkey).getLabel().equals(key))
				{
			...
			for (Element element: survey.getElementsRecursive())
			{
				...
				if (element instanceof Section)
				{
					...
					{
						section.setTabTitle(translationsByKey.get(element.getId().toString() + Section.TABTITLE).getLabel());
					}
				}
				...
				if (element instanceof NumberQuestion)
				{
					...
					{
						number.setUnit(translationsByKey.get(number.getId().toString() + NumberQuestion.UNIT).getLabel());
					} else {
						number.setUnit("");
					}
				}		
				...
				if (element instanceof Confirmation)
				{
					...
					{
						confirmation.setConfirmationlabel(translationsByKey.get(confirmation.getUniqueId() + Confirmation.LABEL).getLabel());
					} 
					if (translationsByKey.get(confirmation.getUniqueId() + Confirmation.TEXT) != null)
					{
						confirmation.setConfirmationtext(translationsByKey.get(confirmation.getUniqueId() + Confirmation.TEXT).getLabel());
					}					
				}	
				...
				if (element instanceof ChoiceQuestion)
				{
					...
					for (PossibleAnswer answer : choice.getPossibleAnswers())
					{
						...
						{
							answer.getScoring().setFeedback(translationsByKey.get(answer.getUniqueId() + Question.FEEDBACK).getLabel());
						}
					}
				}		
				...
				if (element instanceof Question)
				{
					...
					if (survey.getIsQuiz() && question.getScoring() != 0 && question.getScoringItems() != null)
					{
						for (ScoringItem scoringItem: question.getScoringItems())
						{
							...
							{
								scoringItem.setFeedback(translationsByKey.get(scoringItem.getUniqueId() + Question.FEEDBACK).getLabel());
							}
						}
					}
				}	
	...
	public static Survey createTranslatedSurvey(int surveyId, String lang, SurveyService surveyService, TranslationService translationService, boolean setSurvey) {
		
		Survey survey = surveyService.getSurveyReadOnly(surveyId, false, setSurvey);
		
		if (survey != null)
		{
			if (lang.equalsIgnoreCase(survey.getLanguage().getCode())) return survey;
			
			Translations translations = translationService.getTranslations(survey.getId(), lang);
			
			if (translations == null) return survey;
		...
		{			
			if (element instanceof MatrixOrTable || element instanceof GalleryQuestion)
			{
                ...
                if (bf!=null) {
                	...
                	if (!done){
	                	...
	                	for (i=0; i<matrix.getColumns(); i++)
	                	{
	                		...
	                		if (child.getUniqueId() != null || child.getId().toString() != null) {
	                			...
	                			wtext = 0;
	                			if (str!=null) {
						          wtext += bf.getWidthPoint(str, 10) + 2 * 9.0/*matrixtable.td.padding-left + matrixtable.td.padding-right*/ + 1.0 /*border*//*cf. [runner.jsp] */; // FIXME: Remove hardcoding constants from runner.jsp
	                			}
					          if (!isTable)
					          {
	    			...
	    			//	w += 2.0 + 28.34/*page.margin-left + page.margin-right*/ + 18.0/*matrixtable.margin-left*/ + 1.0/*last border*/;
	    			//	w += 100.0 ;
