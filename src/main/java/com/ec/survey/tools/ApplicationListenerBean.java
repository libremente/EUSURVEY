...
public class ApplicationListenerBean implements ApplicationListener<ContextRefreshedEvent> {
	...
	public void onApplicationEvent(ContextRefreshedEvent event) {
		if (event != null) {
			...
			// the event is thrown twice (one for each context), so we only count the last
			// one
			if (applicationContext.getId().endsWith("dispatcher")) {
				...
				initializeDatabase(administrationService, surveyService, schemaService, skinService,
						homeController.servletContext, homeController.fileDir, homeController.serverPrefix,
						homeController.createStressData != null
								&& homeController.createStressData.equalsIgnoreCase("1"),
						showEcas, homeController.sender, fileService, homeController.createStressData != null
								&& homeController.createStressData.equalsIgnoreCase("2"));
	...
	@Transactional
	public static void initializeDatabase(AdministrationService administrationService, SurveyService surveyService,
			SchemaService schemaService, SkinService skinService, ServletContext servletContext, String fileDir,
			String host, boolean createStressTestData, boolean showecas, String sender, FileService fileService,
			boolean createNewStressTestData) {
		...
		if (result.size() == 0) {
			...
			try {
				UsersCreator.createDefaultUsers(administrationService, createStressTestData || createNewStressTestData,
						sender);
			} catch (Exception e1) {
				logger.error(e1);
			}
			...
			try {
				...
				if (createStressTestData) {
					...
					SurveyCreator.createStressTestSurvey(analyst, objLang, servletContext, fileDir, surveyService,
							fileService);
				}

				if (createStressTestData || createNewStressTestData) {
					User analyst = administrationService.getUserForLogin(administrationService.getStressUser(), false);
					SurveyCreator.createStressTestSurveys(analyst, objLang, servletContext, fileDir, surveyService,
							fileService);
				}
	...
	public static Survey createSurvey(int answerCount, User user, Language objLang, SurveyService surveyService,
			AnswerService answerService, String fileDir, boolean init, MessageSource resources, Locale locale,
			Integer questions, boolean archivesurvey, ArchiveService archiveService, BeanFactory context,
			TaskExecutor taskExecutor, FileService fileService) throws Exception {
		...
		if (archivesurvey) {
			...
			archive.setReplies(
					answerService.getNumberOfAnswerSetsPublished(survey.getShortname(), survey.getUniqueId()));
			...
			createDummyAnswers(survey.getShortname(), answerCount, user, fileDir, answerService, surveyService, false,
					resources, locale, fileService);
		}

		return survey;
	}

	public static void createDummyAnswers(String shortname, int answerCount, User user, String fileDir,
			AnswerService answerService, SurveyService surveyService, boolean validate, MessageSource resources,
			Locale locale, FileService fileService) throws Exception {
		...
		for (int j = 0; j < answerCount; j++) {
			...
			if (validate) {
				...
				HashMap<Element, String> validation = SurveyHelper.validateAnswerSet(answerSet, answerService,
						invisibleElements, resources, locale, null, null, true, null, fileService);
			}
	...
	@Transactional
	public static void saveAnswerSet(AnswerSet answerSet, String fileDir, AnswerService answerService, String draftid)
			throws Exception {
		boolean saved = false;
