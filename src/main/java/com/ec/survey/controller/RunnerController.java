...
public class RunnerController extends BasicController {
	...
	public ModelAndView confirmation(HttpServletRequest request, HttpServletResponse response, Locale locale, Model modelMap) throws InvalidURLException {
		...
		Form form = new Form(resources, surveyService.getLanguage(lang), translationService.getActiveTranslationsForSurvey(survey.getId()),contextpath);
	...
	public ModelAndView invited(@PathVariable String group, @PathVariable String unique, HttpServletRequest request, Locale locale, Integer draftSurveyId, Device device) throws WeakAuthenticationException, NotAgreedToPsException, NotAgreedToTosException {
		...
		try {
			...
			if (invitation != null && invitation.getAnswers() > 0) {
				...
				model.addObject("message", resources.getMessage("error.InvitationUsed", null, "The invitation was already used.", locale));
				AnswerSet aws = answerService.getByInvitationCode(invitation.getUniqueId());
				Date answerDate;
				if(aws != null)
				{
					answerDate = aws.getDate();//label.submittedOn
					String[] args = new String[] {ConversionTools.getFullString(answerDate)};
					model.addObject("messageComplement",
							resources.getMessage("message.SubmittedOn", args, "Submitted on: "+ ConversionTools.getFullString(answerDate), locale));
				}
			...
			if (invitation != null && invitation.getDeactivated() != null && invitation.getDeactivated()) {
				ModelAndView model = new ModelAndView("error/generic");
				model.addObject("message", resources.getMessage("error.AccessDeactivated", null, "The access has not yet been activated. Please try again later.", locale));
				model.addObject("noMenu", true);
				model.addObject("runnermode", true);
				return model;
			}
			...
			if (invitation != null && (invitation.getDeactivated() == null || !invitation.getDeactivated()) && participationGroup != null
				...
				if (!participationGroup.getActive()) {
					ModelAndView model = new ModelAndView("error/generic");
					model.addObject("message", resources.getMessage("error.InvitationDeactivated", null, "The access for this guest-list has not yet been activated. Please try again later.", locale));
					model.addObject("noMenu", true);
					model.addObject("runnermode", true);
					return model;
				}
				...
				if (survey != null) {
					...
					if (!draftSurvey.getIsActive() || SurveyHelper.isDeactivatedOrEndDateExceeded(draftSurvey, surveyService)) {
					...
					if (draftid != null && draftid.trim().length() > 0) {
						try {
							...
							SurveyHelper.recreateUploadedFiles(draft.getAnswerSet(), fileDir, f.getSurvey(), fileService);
					...
					if (draft != null) {
						...
						ModelAndView model = new ModelAndView("error/generic");
						model.addObject("message", resources.getMessage("error.SurveyNotActive", null, "The survey is not active at the moment. Please try again later.", locale));
						model.addObject("noMenu", true);
						return model;
					}
				}
		...
		ModelAndView model = new ModelAndView("error/generic");
		model.addObject("message", resources.getMessage("error.WrongURL", null, "The url you entered was not correct.", locale));
		model.addObject("noMenu", true);
		model.addObject("runnermode", true);
		return model;
	}
	...
	@RequestMapping(value = "/invited/{group}/{unique}", method = RequestMethod.POST)
	public ModelAndView invitedPOST(@PathVariable String group, @PathVariable String unique, HttpServletRequest request, Locale locale, Device device) {
		try {
	...
	public ModelAndView runnerToken(@PathVariable String shortname, @PathVariable String token, HttpServletRequest request, Locale locale, Device device) throws InvalidURLException, ForbiddenURLException {
		Survey survey = null;
		...
		try {
			...
			if (invitation != null) {
				...
				if (participationGroup != null && participationGroup.getSurveyUid().equals(survey.getUniqueId())) {
					return invited(participationGroup.getId().toString(), token, request, locale, survey.getId(), device);
				}
			}
	...
	public ModelAndView runner(@PathVariable String uidorshortname, HttpServletRequest request, HttpServletResponse response, Locale locale, Device device) throws InvalidURLException, ForbiddenURLException, WeakAuthenticationException, FrozenSurveyException, NotAgreedToTosException, NotAgreedToPsException {
		...
		if (isShowEcas()) modelReturn.getModelMap().put("showecas", true);
		if (isCasOss()) modelReturn.getModelMap().put("casoss", true);
		...
		boolean isDraft = request.getParameter("draft") != null && request.getParameter("draft").equalsIgnoreCase("true");
				...
				if (survey.getEcasSecurity())
				{
					try {
						...
						if (readonlyMode || (user != null && user.getType().equalsIgnoreCase(User.ECAS) && ecasauthenticated))
						{
							...
							{
								if (survey.getAllowedContributionsPerUser() == 1 || survey.getAllowedContributionsPerUser() <= contributionsCount)
								{								
									...
									if (survey.getAllowedContributionsPerUser() == 1)
									{
										...
										modelReturn.addObject("message", resources.getMessage("error.UserAlreadySubmitted2", null, "You have submitted the maximum number of contributions this survey allows. Further contributions are not allowed.", locale));
									}
							...
							if (readonlyMode || (survey.getEcasMode() != null && survey.getEcasMode().equalsIgnoreCase("all")))
							{
	...
	private ModelAndView getEscapePageModel(Survey survey, HttpServletRequest request, Device device, Boolean escapeOrMaxReachedAnswers) {
		if (escapeOrMaxReachedAnswers && survey.getEscapePageLink() != null && survey.getEscapePageLink() && survey.getEscapeLink() != null && survey.getEscapeLink().length() > 0) {
			...
			ModelAndView model;
			if ((device.isMobile() || device.isTablet()) && (enableresponsive != null && enableresponsive.equalsIgnoreCase("true"))	)
			{
			...
			if (lang != null) {
				...
				if (f.translationIsValid(lang))
				{
					survey = SurveyHelper.createTranslatedSurvey(f.getSurvey().getId(), lang, surveyService, translationService, true);				
					f.setSurvey(survey);
					f.setLanguage(surveyService.getLanguage(lang));
				} else {
					lang = null;
	...
	public @ResponseBody String delete(@PathVariable String id, @PathVariable String uniqueCode, @PathVariable String surveyUID, HttpServletRequest request, HttpServletResponse response) {
		try {
	...
	private String validateDeleteParameters(String id, String uniqueCode, String fileName, String surveyUID) throws ValidationException, IOException {
		...
		boolean validDirectoryPath = validator.isValidDirectoryPath("check directory path in RunnerController.delete method", canonicalPath, fileService.getSurveyUploadsFolder(surveyUID, false), false);
	...
	public void upload(@PathVariable String id, @PathVariable String uniqueCode, HttpServletRequest request, HttpServletResponse response) {
					...
					while (!error)
					{
						try {
							...
							if (counter > 4) {
								...
								logger.error("not possible to copy file to: " + file.getPath() + " - attempt " + counter);
							}
						}
					}
				}
	...
	public ModelAndView processDraftSubmit(@PathVariable String mode, HttpServletRequest request, Locale locale, HttpServletResponse response, Device device) {
		try {
			boolean hibernateOptimisticLockingFailureExceptionCatched = false;
			...
			if (!mode.equalsIgnoreCase("preview"))
			{
				...
				if (draftid != null) {
					...
					if (draft != null) {
						SurveyHelper.parseAndMergeAnswerSet(request, survey, fileDir, uniqueCode, draft.getAnswerSet(), lang, user, fileService);
				...
				if (draft == null) {
					...
					answerSet = SurveyHelper.parseAnswerSet(request, survey, fileDir, uniqueCode, false, lang, user, fileService);
					...
					if (invitation.getAttendeeId() != 0) {
						...
						if (group.getType() == ParticipationGroupType.ECMembers) {
							...
							if (attendee != null)
							{
								...
								logger.error("Attendee " + invitation.getAttendeeId() + " referenced by invitation " + invitation.getId() + " not found!");
							}
						}
					}
			...
			if (request.getParameter("passwordauthenticated") != null && request.getParameter("passwordauthenticated").equalsIgnoreCase("true"))
			{
				if (url.contains("?"))
				{
					url += "&pw=1";
	...
	public ModelAndView DraftSubmit(@PathVariable String draftid, HttpServletRequest request, Locale locale, HttpServletResponse response, Device device) throws Exception {
		...
		if (mode.equalsIgnoreCase("test")) {
			...
			result.addObject("runnermode", true);
			Form f = new Form(resources, surveyService.getLanguage(lang), translationService.getTranslationsForSurvey(survey.getId(), false),contextpath);
	...
	@RequestMapping(value = "/{uidorshortname}", method = RequestMethod.POST)
	public ModelAndView processSubmit(@PathVariable String uidorshortname, HttpServletRequest request, HttpServletResponse response, Locale locale, Model modelMap, Device device) {
		boolean hibernateOptimisticLockingFailureExceptionCatched = false;
		try {

			if (request.getParameter("redirectFromCheckPassword") != null && request.getParameter("redirectFromCheckPassword").equalsIgnoreCase("true") && request.getParameter("password") != null
					&& request.getParameter("shortname") != null) {
				Survey survey = surveyService.getSurvey(uidorshortname, false, true, false, false, null, true, true);
				Survey draftsurvey = surveyService.getSurvey(uidorshortname, true, false, false, false, null, true, false);
				String password = request.getParameter("password");

				if (survey != null) {
					if (survey.getSecurity().startsWith("secured")) {
						if (survey.getPassword() != null && survey.getPassword().trim().length() > 0) {
							if (survey.getPassword().equals(password)) {
								// authenticated
								return loadSurvey(survey, request, response, locale, uidorshortname, true, device, false);
							}
						}
						...
						if (invitation != null) {
							...
							if (participationGroup != null && participationGroup.getSurveyUid().equals(draftsurvey.getUniqueId())) {
								return new ModelAndView("redirect:/runner/invited/" + participationGroup.getId() + "/" + password);
							}
						}
						...
						if (survey.getEcasSecurity())
						{
							...
							if (draftid != null && draftid.length() > 0)
							{
								model.addObject("serviceurl", serverPrefix + "auth/surveylogin?survey=" + uidorshortname + "#" + draftid);
							} else {
			...
			try {
				...
				Form f = new Form(survey, translationService.getActiveTranslationsForSurvey(survey.getId()), survey.getLanguage(), resources, contextpath);
				f.getAnswerSets().add(answerSet);
				validation.put((Element) ie.getElement(), resources.getMessage("error.InvalidEmail", null, "The email address is not valid", locale));
				f.setValidation(validation);
				ModelAndView model = new ModelAndView("runner/runner", "form", f);
				surveyService.initializeSkin(f.getSurvey());			
				model.addObject("submit", true);
				...
				model.addObject("submit", true);
				model.addObject("uniqueCode", uniqueCode);
				model.addObject("message", resources.getMessage("error.LoginExists", null, "This login already exists. Please choose a unique login.", locale));
	...
	@RequestMapping(value = "/createdraftanswerpdf/{code}", headers = "Accept=*/*", method = {RequestMethod.GET, RequestMethod.HEAD})
	...
	@RequestMapping(value = "/createquizpdf/{code}", headers = "Accept=*/*", method = {RequestMethod.GET, RequestMethod.HEAD})
	public @ResponseBody String createquizpdf(@PathVariable String code, HttpServletRequest request, HttpServletResponse response) {
		try {
	...
	public @ResponseBody String sendDraftLinkByMail(HttpServletRequest request, HttpServletResponse response)
	{
		...
		else
		{
			...
			String body = "Dear EUSurvey user,<br /><br />Your contribution to the survey '<b>" + survey.cleanTitle() + "</b>' has been successfully saved as a draft. To retrieve your draft, please follow this link:<br /><br />";
			...
			try {
				...
				mailService.SendHtmlMail(email, sender, sender, 
						resources.getMessage("message.mail.linkDraftSubject", null, request.getLocale()), 
						text, smtpServer, Integer.parseInt(smtpPort), null);
			} catch (Exception e) {
	...
	public @ResponseBody List<Element> element(@PathVariable String id, HttpServletRequest request, HttpServletResponse response) throws NotAgreedToTosException, WeakAuthenticationException, NotAgreedToPsException {
		...
		Survey survey = SurveyHelper.createTranslatedSurvey(Integer.parseInt(id), slang, surveyService, translationService, false);
		
		if (survey == null) return null;
		
		boolean foreditor = request.getParameter("foreditor") != null && request.getParameter("foreditor").equalsIgnoreCase("true");
